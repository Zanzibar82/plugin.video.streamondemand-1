...
def mainlist(item):
    ...
    if 2 in estados:
        itemlist.insert(0, Item(channel=item.channel, action="clean_ready", title="Rimuovere downloads completati",
                                contentType=item.contentType, contentChannel=item.contentChannel,
                                contentSerieName=item.contentSerieName))

    # Si hay alguno con error
    if 3 in estados:
        itemlist.insert(0, Item(channel=item.channel, action="restart_error", title="Riavvio downloads falliti",
                                contentType=item.contentType, contentChannel=item.contentChannel,
                                contentSerieName=item.contentSerieName))

    # Si hay alguno pendiente
    if 1 in estados or 0 in estados:
        itemlist.insert(0, Item(channel=item.channel, action="download_all", title="Scaricare tutti",
                                contentType=item.contentType, contentChannel=item.contentChannel,
                                contentSerieName=item.contentSerieName))

    if len(itemlist):
        itemlist.insert(0, Item(channel=item.channel, action="clean_all", title="Rimuovere tutti",
                                contentType=item.contentType, contentChannel=item.contentChannel,
                                contentSerieName=item.contentSerieName))

    if not item.contentType == "tvshow" and config.get_setting("browser", "descargas") == True:
        itemlist.insert(0, Item(channel=item.channel, action="browser", title="Vesioni download", url = DOWNLOAD_PATH))
...
def move_to_libray(item):
    ...
    if filetools.isfile(destino):
        if item.contentType == "movie" and item.infoLabels["tmdb_id"]:
            library_item = Item(title="Descargado: %s" % item.downloadFilename, channel="descargas",
                                action="findvideos", infoLabels=item.infoLabels, url=item.downloadFilename)
...
def save_server_statistics(server, speed, success):
    ...
    if os.path.isfile(STATS_FILE):
        servers = jsontools.load_json(open(STATS_FILE, "rb").read())
    else:
        servers = {}
