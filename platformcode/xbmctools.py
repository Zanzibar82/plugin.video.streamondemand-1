...
def add_new_folder(item, totalItems=0):
    logger.info('streamondemand.platformcode.xbmctools add_new_folder item=' + item.tostring())
    ...
    logger.info("streamondemand.platformcode.xbmctools add_new_folder itemurl=" + itemurl)
    ...
    if "1" in item.context and accion != "por_teclado":
        DeleteCommand = "XBMC.RunPlugin(%s?%s)" % (
        sys.argv[0], item.clone(channel="buscador", action="borrar_busqueda").tourl())
    if "4" in item.context:
        searchSubtitleCommand = "XBMC.RunPlugin(%s?%s)" % (
        sys.argv[0], item.clone(channel="subtitletools", action="searchSubtitle").tourl())
    if "5" in item.context:
        trailerCommand = "XBMC.RunPlugin(%s?%s)" % (
        sys.argv[0], item.clone(channel="trailertools", action="buscartrailer", contextual=True).tourl())
...
def play_video(item, desdefavoritos=False, desdedescargados=False, desderrordescargas=False, strmfile=False):
    ...
    if opciones[seleccion] == config.get_localized_string(30158):  # "Enviar a JDownloader"
        ...
        if item.subtitle != "":
            data = scrapertools.cachePage(config.get_setting(
                "jdownloader") + "/action/add/links/grabber0/start1/web=" + item.url + " " + item.thumbnail + " " + item.subtitle)
        else:
            data = scrapertools.cachePage(config.get_setting(
                "jdownloader") + "/action/add/links/grabber0/start1/web=" + item.url + " " + item.thumbnail)
    ...
    # Descargar
        ...
        advertencia = xbmcgui.Dialog()
        resultado = advertencia.ok(config.get_localized_string(30102), item.title,
        ...
        advertencia = xbmcgui.Dialog()
        resultado = advertencia.ok(config.get_localized_string(30101), item.title,
        ...
        if keyboard.isConfirmed():
            ...
            favoritos.savebookmark(titulo=download_title, url=item.url, thumbnail=download_thumbnail,
                                   server=item.server, plot=download_plot, fulltitle=item.title)
            advertencia = xbmcgui.Dialog()
            resultado = advertencia.ok(config.get_localized_string(30102), item.title,
        ...
        if keyboard.isConfirmed():
            ...
            descargas.savebookmark(titulo=download_title, url=item.url, thumbnail=download_thumbnail,
                                   server=item.server, plot=download_plot, fulltitle=download_title)

            advertencia = xbmcgui.Dialog()
            resultado = advertencia.ok(config.get_localized_string(30101), download_title,
    ...
    if strmfile:
        ...
        xlistitem.setInfo("video",
                          {"Title": play_title, "Plot": play_plot, "Studio": item.channel, "Genre": item.category})
    ...
    # Movido del conector "torrent" aqui
        ...
        if xbmc.getCondVisibility('System.HasAddon("plugin.video.torrenter")'):
            torrent_options.append(
                ["Plugin externo: torrenter", "plugin://plugin.video.torrenter/?action=playSTRM&url=%s"])
    ...
    if seleccion == 1:
        ...
        mct.play(mediaurl, xbmcgui.ListItem("", iconImage=item.thumbnail, thumbnailImage=item.thumbnail),
        ...
        if config.get_setting("player_mode") == "3" or "megacrypter.com" in mediaurl:
            ...
            if item.channel == "cuevana" and item.subtitle != "":
                ...
                if item.subtitle != "" and (
                    opciones[seleccion].startswith("Ver") or opciones[seleccion].startswith("Watch")):
...
def getLibraryInfo(mediaurl):
    ...
    '''
    if DEBUG:
        logger.info('streamondemand.platformcode.xbmctools playlist OBTENCIÓN DE DATOS DE BIBLIOTECA')
...
def alertnodisponibleserver(server):
    ...
    resultado = advertencia.ok(config.get_localized_string(30055), (config.get_localized_string(30057) % server),
...
def alertanomegauploadlow(server):
    ...
    resultado = advertencia.ok(config.get_localized_string(30055), config.get_localized_string(30061),
                               config.get_localized_string(30062))
...
def playstrm(params, url, category):
    ...
    '''
    logger.info("streamondemand.platformcode.xbmctools playstrm url=" + url)
    ...
    play_video("Biblioteca streamondemand", server, url, category, title, thumbnail, plot, strmfile=True, Serie=serie,
               subtitle=subtitle)
...
def wait2second():
    logger.info("streamondemand.platformcode.xbmctools wait2second")
    import time
    contador = 0
    while xbmc.Player().isPlayingVideo() == False:
        logger.info(
            "streamondemand.platformcode.xbmctools setSubtitles: Waiting 2 seconds for video to start before setting subtitles")
        time.sleep(2)
        contador = contador + 1
...
def setSubtitles():
    logger.info("streamondemand.platformcode.xbmctools setSubtitles")
    import time
    contador = 0
    while xbmc.Player().isPlayingVideo() == False:
        logger.info(
    ...
    logger.info("streamondemand.platformcode.xbmctools setting subtitle file %s" % subtitlefile)
...
def trailer(item):
    ...
    xbmc.executebuiltin(
        ...
        sys.argv[0], "trailertools", "buscartrailer", urllib.quote_plus(item.category),
        urllib.quote_plus(item.fulltitle), urllib.quote_plus(item.url), urllib.quote_plus(item.thumbnail),
        urllib.quote_plus("")))
    return
